...
"""
论文地址：INCORPORATING BERT INTO NEURAL MACHINE TRANSLATION
只实现了encoder
"""
...
class PretrainAttnTransModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        self.attn_alpha = nn.Parameter(torch.FloatTensor(1).fill_(1).float(), requires_grad=True)
        ...
        self.pretrain_attn = nn.ModuleList()
        ...
        for layer_id in range(self.n_layers):
            ...
            self.pretrain_attn.append(MultiHeadAttention(self.n_heads, self.dim, dropout=self.attention_dropout))
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None,
        ...
        for i in range(self.n_layers):
            ...
            # pretrain attention
            pretrain_attn = self.pretrain_attn[i](tensor, attn_mask, kv=pretrain_output)
            pretrain_attn = F.dropout(pretrain_attn, p=self.dropout, training=self.training)

            attn = attn + self.attn_alpha * pretrain_attn
            tensor = tensor + attn
